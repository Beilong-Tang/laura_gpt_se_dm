...
from funcodec.bin.text2audio_inference import Text2Audio, save_audio, torchaudio
from funcodec.tasks.text2audio_generation import Text2AudioGenTask
from utils.utils import AttrDict, setup_logger, update_args, setup_seed
...
def inference(rank, args):
    ...
    with torch.no_grad(), tqdm.tqdm(scp, desc="[inferencing...]") as pbar:
        for audio_path in pbar:
            ...
            # 1. Inference
            start = time.time()
            output = sp_en(mel)[0]["gen"].squeeze(0)  # [1,T]
            rtf = (time.time() - start) / (output.size(1) / sr)
